...
class Auth extends Model
{
     ...
     * @return bool
     */
    public function login(string $cnpjCpf, string $password, bool $save = false, int $level = 1): bool
    {
        /*if (!is_email($email)) {
        ...
        if ($save) {
            setcookie("authCnpjcpf", $cnpjCpf, time() + 604800, "/");
        } else {
            setcookie("authCnpjcpf", null, time() - 3600, "/");
        }
        ...
        $user = (new User())->findByDocument($cnpjCpf);
        if (!$user) {
            $this->message->error("O CNPJ/CPF informado não está cadastrado");
            return false;
        }
