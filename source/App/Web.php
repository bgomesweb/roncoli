...
use Source\App\Admin\Slide;
use Source\Core\Controller;
use Source\Core\View;
use Source\Models\Auth;
use Source\Models\Categories;
use Source\Models\Product;
use Source\Models\PostCatalog;
...
class Web extends Controller
{
    ...
    public function home(): void
    {
        ...
        echo $this->view->render("home", [
            "head" => $head,
            "slides" => (new PostSlide())->find()->fetch(true),
            "company" => (new MyCompany())->find()->order("id DESC")->limit(1)->fetch(true)
        ]);
    }
    ...
    public function history(): void
    {
        ...
        $history = (new Histories())->findPost();

        echo $this->view->render("history", [
            "head" => $head,
            "history" => $history->order("id")->fetch(true)
        ]);
    }
    ...
    public function catalog(?array $data): void
    {
        ...
        $catalog = (new PostCatalog())->find();
        $pager = new Pager(url("/catalogos/p/"));
        $pager->pager($catalog->count(), 12, ($data['page'] ?? 1));

        echo $this->view->render("catalog", [
            "head" => $head,
            "catalog" => $catalog->order("post_at ASC")->limit($pager->limit())->offset($pager->offset())->fetch(true),
     ...
     * @param array $data
     */
    public function catalogSearch(array $data): void
    {
        if (!empty($data['s'])) {
            $search = str_search($data['s']);
            echo json_encode(["redirect" => url("/catalogos/buscar/{$search}/1")]);
            return;
        }
        ...
        $head = $this->seo->render(
            ...
            url("/catalogos/buscar/{$search}/{$page}"),
            theme("/assets/images/share.jpg")
        );

        $catalogSearch = (new PostCatalog())->findPost("MATCH(title) AGAINST(:s)", "s={$search}");

        if (!$catalogSearch->count()) {
            echo $this->view->render("catalog", [
                "head" => $head,
                "title" => "PESQUISA POR:",
                "search" => $search
        ...
        $pager = new Pager(url("/catalogos/buscar/{$search}/"));
        $pager->pager($catalogSearch->count(), 9, $page);

        echo $this->view->render("catalog", [
            "head" => $head,
            "title" => "PESQUISA POR:",
            "search" => $search,
            "catalog" => $catalogSearch->limit($pager->limit())->offset($pager->offset())->fetch(true),
            "paginator" => $pager->render()
        ]);
    }
    ...
    public function categories(?array $data): void
    {
        ...
        $category = (new Categories())->find();
        $pager = new Pager(url("/produtos/p/"));
        $pager->pager($category->count(), 9, ($data['page'] ?? 1));

        echo $this->view->render("products", [
            "head" => $head,
            "category" => $category->order("title DESC")->limit($pager->limit())->offset($pager->offset())->fetch(true),
            "paginator" => $pager->render()
        ]);
    }

    /**
     * SITE SUB CATEGORIA PRODUTOS
     * @param array $data
     */
    public function products(array $data): void
    {
        ...
        $products = (new Product())->find("category = :category", "category={$category}")->fetch(true);
    ...
    public function contactForm(array $data): void
    {
        ...
        $subject = filter_var($data["subject"]);
        ...
        )->send(CONF_MAIL_SUPPORT, "Contato - Roncoli");
    ...
    {
        $head = $this->seo->render(
            "Eventos - " . CONF_SITE_NAME,
            "Confira nossos eventos, e fique por dentro.",
            url("/blog"),
            theme("/assets/images/share.jpg")
        );
    ...
    public function blogCategory(array $data): void
    {
        ...
        $head = $this->seo->render(
            "Eventos em {$category->title} - " . CONF_SITE_NAME,
    ...
    public function login(?array $data): void
    {
        ...
        if (!empty($data['csrf'])) {
            ...
            if (empty($data['document']) || empty($data['password'])) {
                $json['message'] = $this->message->warning("Informe seu CNPJ/CPF e senha para entrar")->render();
                echo json_encode($json);
                return;
            }
            ...
            $auth = new Auth();
            $login = $auth->login($data['document'], $data['password'], $save);

            if ($login) {
                $this->message->success("Seja bem-vindo(a) de volta " . Auth::user()->company_name . "!")->flash();
    ...
    public function confirm(): void
    {
        ...
        echo $this->view->render("optin", [
            ...
            "data" => (object)[
                ...
                "desc" => "Enviamos um link de confirmação para seu e-mail. Acesse e siga as instruções para concluir seu cadastro e tenha acesso aos seu boletos.",
    ...
    public function success(array $data): void
    {
        ...
        $company = (new User())->findByEmail($email);
        ...
        echo $this->view->render("optin", [
            "head" => $head,
            "data" => (object)[
                "title" => "Tudo pronto. Você já pode imprimir seus boletos :)",
                "desc" => "Bem-vindo(a) ao seu painel da Roncoli",
