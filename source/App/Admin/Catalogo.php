...
class Catalogo extends Admin
{
    /**
     * Catálogo constructor.
     */
    ...
    public function home(?array $data): void
    {
        ...
        if (!empty($data["s"])) {
            $s = str_search($data["s"]);
            echo json_encode(["redirect" => url("/admin/catalogos/home/{$s}/1")]);
            return;
        }

        $search = null;
        $posts = (new PostCatalog())->find();
        ...
        if (!empty($data["search"]) && str_search($data["search"]) != "all") {
            ...
            if (!$posts->count()) {
                ...
                redirect("/admin/catalogos/home");
            }
        }

        $all = ($search ?? "all");
        $pager = new Pager(url("/admin/catalogos/home/{$all}/"));
        $pager->pager($posts->count(), 12, (!empty($data["page"]) ? $data["page"] : 1));

        $head = $this->seo->render(
            CONF_SITE_NAME . " | Catálogo",
            CONF_SITE_DESC,
            url("/admin"),
        ...
        echo $this->view->render("widgets/catalogos/home", [
            "app" => "catalogos/home",
            "head" => $head,
    ...
    public function post(?array $data): void
    {
        ...
        if (!empty($data["action"]) && $data["action"] == "create") {
            ...
            if (!empty($_FILES["cover"])) {
                $files = $_FILES["cover"];
                $upload = new Upload();
                $image = $upload->file($files, $postCreate->title);
            ...
            $this->message->success("Catálogo publicado com sucesso...")->flash();
            $json["redirect"] = url("/admin/catalogos/post/{$postCreate->id}");
        ...
        if (!empty($data["action"]) && $data["action"] == "update") {
            //$content = $data["content"];
            $data = filter_var_array($data, FILTER_SANITIZE_STRIPPED);
            $postEdit = (new PostCatalog())->findById($data["post_id"]);

            if (!$postEdit) {
                $this->message->error("Você tentou atualizar um post que não existe ou foi removido")->flash();
                echo json_encode(["redirect" => url("/admin/catalogos/home")]);
                return;
            }
            ...
            if (!empty($_FILES["cover"])) {
                ...
                $files = $_FILES["cover"];
                $upload = new Upload();
                $image = $upload->file($files, $postEdit->title);
        ...
        if (!empty($data["action"]) && $data["action"] == "delete") {
            $data = filter_var_array($data, FILTER_SANITIZE_STRIPPED);
            $postDelete = (new PostCatalog())->findById($data["post_id"]);

            if (!$postDelete) {
                $this->message->error("Você tentou excluir um catálogo que não existe ou já foi removido")->flash();
                echo json_encode(["reload" => true]);
                return;
            }
            ...
            $postDelete->destroy();
            $this->message->success("O catálogo foi excluído com sucesso...")->flash();
        ...
        if (!empty($data["post_id"])) {
            ...
            $postEdit = (new PostCatalog())->findById($postId);
        }

        $head = $this->seo->render(
            CONF_SITE_NAME . " | " . ($postEdit->title ?? "Novo Catálogo"),
            CONF_SITE_DESC,
            url("/admin"),
            url("/admin/assets/images/image.jpg"),
            false
        ...
        echo $this->view->render("widgets/catalogos/post", [
            "app" => "catalogos/post",
            "head" => $head,
            "post" => $postEdit,
