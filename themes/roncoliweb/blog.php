...
<section class="blog_page">
    <header class="blog_page_header">
        <h1><?= ($title ?? "EVENTOS"); ?></h1>
        <p><?= ($search ?? $desc ?? "Confira nossos eventos e fique por dentro!"); ?></p>
        <form name="search" action="<?= url("/blog/buscar"); ?>" method="post" enctype="multipart/form-data">
            <label>
                <input type="text" name="s" placeholder="Encontre um evento:" required/>
        ...
        <div class="content content">
            <div class="empty_content">
                ...
                <a class="empty_content_btn gradient gradient-green gradient-hover radius"
                   href="<?= url("/blog"); ?>" title="Eventos">...ou volte aos eventos</a>
            </div>
        </div>
    <?php elseif (empty($blog)): ?>
        <div class="content content">
            <div class="empty_content">
                ...
                <p class="empty_content_desc">Nossos editores estão preparando novos eventos para você.</p>
            </div>
        </div>
    <?php else: ?>
