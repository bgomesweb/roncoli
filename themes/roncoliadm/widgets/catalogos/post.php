...
<div class="mce_upload" style="z-index: 997">
    <div class="mce_upload_box">
        <form class="app_form" action="<?= url("/admin/catalogos/post"); ?>" method="post" enctype="multipart/form-data">
...
<section class="dash_content_app">
    <?php if (!$post): ?>
        <header class="dash_content_app_header">
            <h2 class="icon-plus-circle">Novo Catálogo</h2>
        </header>

        <div class="dash_content_app_box">
            <form class="app_form" action="<?= url("/admin/catalogos/post"); ?>" method="post">
                <!-- ACTION SPOOFING-->
                <input type="hidden" name="action" value="create"/>

                <label class="label">
                    <span class="legend">*Título:</span>
                    <input type="text" name="title" placeholder="Nome do catálogo" required/>
                </label>

                <label class="label">
                    <span class="legend">Arquivo:</span>
                    <input type="file" name="cover" placeholder="Um arquivo pdf"/>
                </label>
    ...
    <?php else: ?>
        <header class="dash_content_app_header">
            ...
            <a class="icon-link btn btn-green" href="<?= url("/catalogos/{$post->uri}"); ?>" target="_blank" title="">Ver no
                site</a>
        </header>

        <div class="dash_content_app_box">
            <form class="app_form" action="<?= url("/admin/catalogos/post/{$post->id}"); ?>" method="post">
                <!-- ACTION SPOOFING-->
                <input type="hidden" name="action" value="update"/>

                <label class="label">
                    <span class="legend">*Título:</span>
                    <input type="text" name="title" value="<?= $post->title; ?>" placeholder="Nome do Catálogo"
                           required/>
                </label>
